...
//-------------------------------------------------------------------
const InputCarNum = () => { // 리엑트 함수형 컴포넌트 생성
  ...
  const onSubmit = () => {
    ...
    else { // 올바르지 않은 값이 제출되면 toast와 tts알림으로 반려한다.
      toast("올바른 값을 넣어주세요!");
      tts("올바른 값을 넣어주세요!");
    }
  };
    ...
    <>
      <span className={css.inputField}>
        ...
        <InputField
          ...
          onChange={(ev) => {setCarNumber(ev.value)}}
          value={carNumber}
        />
      </span>
      <DatePicker onChange={setStart}/>
      <DatePicker onChange={setEnd}/>
